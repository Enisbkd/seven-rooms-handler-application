package com.sbm.sevenrooms.sevenroomshandler.logs;
...
public abstract class KafkaLogbackConfigBase<I extends DeferredProcessingAware> extends UnsynchronizedAppenderBase<ILoggingEvent> {
    ...
    protected Properties getProducerProperties() {
        ...
        if (brokerList != null) props.put(BOOTSTRAP_SERVERS_CONFIG, brokerList);
        if (props.isEmpty()) throw new ConfigException("The bootstrap servers property should be specified");
        if (topic == null) throw new ConfigException("Topic must be specified by the Kafka Logback appender");
        if (compressionType != null) props.put(COMPRESSION_TYPE_CONFIG, compressionType);
        ...
        if (
            securityProtocol != null && securityProtocol.contains("SSL") && sslTruststoreLocation != null && sslTruststorePassword != null
        ) {
        ...
        if (
            securityProtocol != null && securityProtocol.contains("SASL") && saslKerberosServiceName != null && clientJaasConfPath != null
        ) {
